...
# Configuration: .wingman/config/hooks-config.json
...
BYPASS_ENV_VAR="${WINGMAN_SKIP_HOOKS:-}"
...
if [ -n "$BYPASS_ENV_VAR" ]; then
    log_warn "WINGMAN_SKIP_HOOKS is set - bypassing pre-commit checks"
...
# =============================================================================
if [ -f ".wingman/config/hooks-config.json" ]; then
    SETUP_COMPLETE=$(grep -o '"setupComplete"[[:space:]]*:[[:space:]]*true' .wingman/config/hooks-config.json || echo "false")

    if [ "$SETUP_COMPLETE" = "false" ]; then
        log_info "Git hooks inactive (no tests configured yet)"
        log_info "Run /wingman:update-git-hooks after adding tests"
        exit 0
    fi
fi
...
if [ -n "$FORMAT_COMMAND" ] && [ "$FORMAT_COMMAND" != "{{FORMAT_COMMAND}}" ]; then
    ...
    if eval "$FORMAT_COMMAND"; then
        ...
        log_error "  WINGMAN_SKIP_HOOKS=1 git commit -m 'message'"
        exit 1
    fi
else
...
if [ -n "$UNIT_TEST_COMMAND" ] && [ "$UNIT_TEST_COMMAND" != "{{UNIT_TEST_COMMAND}}" ]; then
    ...
    if eval "$UNIT_TEST_COMMAND"; then
        ...
        log_error "  WINGMAN_SKIP_HOOKS=1 git commit -m 'WIP: failing test for feature X'"
        log_error ""
