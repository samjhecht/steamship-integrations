...
if [ -n "$BYPASS_ENV_VAR" ]; then
    log_warn "WINGMAN_SKIP_HOOKS is set - bypassing pre-push checks"
...
if [ -z "$TEST_COMMAND" ] || [ "$TEST_COMMAND" = "{{TEST_COMMAND}}" ]; then
    ...
    log_warn "Configure via .wingman/config/hooks-config.json"
    exit 0
fi
...
if [ "$protected_push" = true ]; then
    ...
    if eval "$TEST_COMMAND"; then
        ...
        log_error "  WINGMAN_SKIP_HOOKS=1 git push"
        log_error ""
