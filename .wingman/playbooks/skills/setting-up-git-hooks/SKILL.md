...
- **Pattern A (Default)**: Hooks installed directly to `.git/hooks/` with configuration in `.wingman/config/hooks-config.json`
- **Pattern B**: Version-controlled hooks in `.wingman/config/git-hooks/` with install script for team synchronization
...
# Check for existing wingman hooks config
[ -f .wingman/config/hooks-config.json ] && echo "Existing config found" || echo "No existing config"
```
...
1. **Populate TESTING.md** with placeholder content:
   ...
   **Status:** No tests configured yet
   ...
   If TESTING.md doesn't exist, create it from the testing.md template in `.wingman/playbooks/templates/` or builtins first.
...
2. **Create stub hooks-config.json:**
   ```json
   {
     ...
     "testCommand": "",
     "note": "No tests detected. Run /wingman:updating-git-hooks after adding tests.",
     "protectedBranches": ["main", "master", "feature/*", "fix/*"],
     "skipDocsOnlyChanges": true,
     "docsPatterns": ["*.md", "docs/**/*", ".wingman/notes/**/*"],
     "bypassEnvVar": "WINGMAN_SKIP_HOOKS",
     "setupComplete": false
   }
   ```

   Note: File should be created at `.wingman/config/hooks-config.json`
...
4. **Inform user:**
   ...
   2. Run: /wingman:updating-git-hooks

   Created:
   - .wingman/TESTING.md (placeholder)
   - .wingman/config/hooks-config.json (stub configuration)
    ...
    options: [
      {
        ...
        description: "Hooks in .git/hooks/, config in .wingman/config/hooks-config.json"
      },
      {
        label: "Pattern B: Version-controlled",
        description: "Hooks in .wingman/config/git-hooks/, install script for team sync"
      }
    ],
    multiSelect: false
  }]
...
Use Write tool to create `.wingman/config/hooks-config.json`:

```json
{
  "$schema": "https://wingman.dev/schemas/hooks-config.json",
  "version": "1.0.0",
...
Use Read tool to read hook template files from this skill's `templates/` directory:
- `pre-commit.template.sh`
- `pre-push.template.sh`
- `commit-msg.template.sh` (if enabled)
...
mkdir -p .wingman/config/git-hooks
```

Use Write tool to save hooks to `.wingman/config/git-hooks/`:
- `.wingman/config/git-hooks/pre-commit`
- `.wingman/config/git-hooks/pre-push`
- `.wingman/config/git-hooks/commit-msg` (if enabled)

Use Write tool to create install script at `scripts/install-hooks.sh`:
(Copy from this skill's `templates/install-hooks.sh`)

Make files executable:
```bash
chmod +x .wingman/config/git-hooks/pre-commit
chmod +x .wingman/config/git-hooks/pre-push
[ -f .wingman/config/git-hooks/commit-msg ] && chmod +x .wingman/config/git-hooks/commit-msg
...
- `.github/pull_request_template.md` (copy from this skill's `templates/pull_request_template.md`)

**Note on other templates**: Security checklist and Definition of Done templates remain in their skill directories:
- Security checklist: `.wingman/playbooks/templates/security-checklist.md` (or builtins)
- Definition of Done: `.wingman/playbooks/templates/definition-of-done.md` (or builtins)
...
If `.wingman/TESTING.md` doesn't exist (governance not initialized), create it from initializing-governance template with user's configuration:
